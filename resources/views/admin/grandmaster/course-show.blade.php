            ...
            <div class="p-4 bg-white rounded-2xl shadow-md mt-4">
                ...
                <div class="w-full grid grid-cols-2 gap-6 px-6">
                    ...
                    <fieldset class="fieldset w-full">
                        ...
                        <input class="input rounded-xl w-full validator placeholder:text-gray-500 placeholder:italic" name="par" type="number" value="{{ old('par', $data->par) }}" title="Must be between be 0 to 50" required placeholder="Hole" min="0" max="50" />
                    </fieldset>
                    <fieldset class="fieldset w-full">
                        <legend class="fieldset-legend">Hole Total<span class="text-red-700">*</span></legend>
                        <input class="input rounded-xl w-full validator placeholder:text-gray-500 placeholder:italic" name="total_holes" type="number" value="{{ old('total_holes', $data->total_holes) }}" title="Must be between be 0 to 20" required placeholder="Hole" min="0" max="20" />
                    </fieldset>
                </div>
                ...
                <div class="w-full grid grid-cols-3 gap-12 px-6 pb-2">
                    <div class="rounded-2xl bg-white shadow-[0px_0px_4px_0px_rgba(0,0,0,0.2)] relative" v-for="(item, index) in holes" :key="index">
                        ...
                        <div class="flex items-center justify-start gap-1 flex-wrap p-4">
                            <input type="hidden" :name="'holes[' + index + '][id]'" v-model="holes[index].id" />
                            <fieldset class="fieldset w-full">
                                <legend class="fieldset-legend">Par <span class="text-red-700">*</span></legend>
                                <input class="input rounded-xl w-full validator placeholder:text-gray-500 placeholder:italic" type="number" title="Must be between be 0 to 10" :name="'holes[' + index + '][par]'" required placeholder="Hole" min="0" max="50" v-model="holes[index].par" />
                            </fieldset>
                            <fieldset class="fieldset w-full">
                                <legend class="fieldset-legend">Allowed Time <span class="text-red-700">*</span></legend>
                                <input class="input rounded-xl w-full validator placeholder:text-gray-500 placeholder:italic" type="number" title="Must be between be 0 to 60" :name="'holes[' + index + '][allowed_time]'" required placeholder="Allowed Time" min="0" max="60" v-model="holes[index].allowed_time" />
                            </fieldset>
                        </div>
                    </div>
                </div>
