...
@section('page-content')
    <div class="w-full max-h-screen h-full overflow-auto bg-gray-100" id="app">
        <form class="w-full px-4 pb-4" action="{{ route('setting.update', $data->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="p-4 bg-white rounded-2xl shadow-md mt-4">
                ...
                <div class="w-full grid grid-cols-1 gap-2">
                    <fieldset class="fieldset w-full">
                        ...
                        <input class="input rounded-xl w-full placeholder:text-gray-500 placeholder:italic" name="name" type="text" value="{{ old('name', $data->name) }}" required placeholder="Event Name" />
                    </fieldset>
                    <fieldset class="fieldset w-full">
                        <legend class="fieldset-legend">Periode <span class="text-red-700">*</span></legend>
                        <div class="grid grid-cols-2 gap-2">
                            <input class="input rounded-xl shadow w-full placeholder:text-gray-400" id="date_start" name="date_start" type="text" placeholder="Start Date" required value="{{ old('date_start', $data->date_start) }}"/>
                            <input class="input rounded-xl shadow w-full placeholder:text-gray-400" id="date_end" name="date_end" type="text" placeholder="End Date" required value="{{ old('date_end', $data->date_end) }}"/>
                        </div>
                    </fieldset>
                </div>
            </div>
