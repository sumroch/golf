    ...
    <script>
        const app = Vue.createApp({
            ...
            methods: {
                ...
                handleStartTimeModal() {
                    this.editStart.morning_hour = this.round.morning_hour;
                    this.editStart.morning_minute = this.round.morning_minute;
                    this.editStart.afternoon_hour = this.round.afternoon_hour;
                    this.editStart.afternoon_minute = this.round.afternoon_minute;

                    edit_start_modal.show();
                },
    ...
    <div class="max-h-screen p-4 bg-base-200 h-full overflow-auto w-full" id="app">
        <div class="w-full flex flex-wrap justify-start items-start gap-4">
            <div class="p-6 w-full bg-white rounded-2xl shadow-md flex items-stretch justify-between">
                ...
                <div class="w-auto">
                    <select class="select mb-2 rounded-lg cursor-pointer" v-model="activeRound" @change="changeround()" v-if="round.status != 'active'">
                    ...
                    <div v-if="round.status === 'active'">
                        <div class="grid grid-cols-2 gap-4">
                            <button class="bg-white text-center inline-block rounded-lg px-6 py-3 border border-green-600 hover:bg-green-700 hover:text-white cursor-pointer group/edit" v-on:click="handleStartTimeModal()">
                                <img class="w-6 h-6 inline-block" src="{{ asset('img/icon/clock.svg') }}" alt="">
                                <p class="mt-1.5">Edit Start</p>
                            </button>
                            <button class="bg-white text-center rounded-lg px-6 py-3 border border-red-600 hover:bg-red-700 hover:text-white cursor-pointer group/pause" onclick="pause_modal.show()">
                                <div class="flex items-center justify-center">
                                    <div class="h-5 bg-gray-500 group-hover/pause:bg-white w-2 rounded-sm me-1"></div>
                                    <div class="h-5 bg-gray-500 group-hover/pause:bg-white w-2 rounded-sm"></div>
                                </div>
                                <p class="mt-1.5">Pause</p>
                            </button>
                        </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-md p-4 max-h-175 overflow-auto">
                        ...
                        <div class="grid grid-cols-1 gap-3">
                            <div class="w-full collapse collapse-arrow rounded-xl bg-base-100 border-base-300 border" v-for="(items, key) in holes" :key="key">
                                <input type="checkbox" />
                                <div class="collapse-title py-2 text-lg rounded-none" :class="{ 'text-white bg-linear-to-t to-green-700 from-green-500': items[0]?.head == true, 'text-white bg-linear-to-t to-blue-700 from-blue-500': items[items.length - 1]?.tail == true }">
                                    <span v-text="items[0].keys"></span>
                                    <span class="ms-2" v-if="items[0].head == true">HEAD</span>
                                    <span class="ms-2" v-if="items[items.length - 1].tail == true">TAIL</span>
                                </div>
                                <div class="collapse-content bg-gray-100 pb-0 text-sm px-0 row-start-3 col-start-1">
                                    <div class="w-full flex items-center justify-between border-b border-gray-300 py-2 px-4 font-bold" v-for="item in items" :key="item.id">
                                        <p v-text="item.name"></p>
            ...
            <dialog class="modal" id="edit_start_modal">
                <div class="modal-box max-h-11/12">
                    <form method="dialog">
                        <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
                    </form>
                    <form class="w-full pt-3" :action="'{{ url('admin/reset') }}' + '/' + round.id" method="GET">
                        <p class="text-lg">Edit Start Time</p>
                        <hr class="my-4 border-gray-300">
                        <p class="text-sm mb-2">Morning Session</p>
                        <div class="flex items-center w-full">
                            <select class="input flex-1 rounded shadow w-full placeholder:text-gray-400 select cursor-pointer" id="morning_hour" name="morning_hour" v-model="editStart.morning_hour" required>
                                <option disabled selected>Pick Hour</option>
                                @foreach (range(0, 23) as $item)
                                    <option value="{{ str_pad($item, 2, '0', STR_PAD_LEFT) }}">{{ str_pad($item, 2, '0', STR_PAD_LEFT) }}</option>
                                @endforeach
                            </select>
                            <span class="px-3">:</span>
                            <select class="input flex-1 rounded shadow w-full placeholder:text-gray-400 select cursor-pointer" id="morning_minute" name="morning_minute" v-model="editStart.morning_minute" required>
                                <option disabled selected>Pick Minute</option>
                                @foreach (range(0, 59) as $item)
                                    <option value="{{ str_pad($item, 2, '0', STR_PAD_LEFT) }}">{{ str_pad($item, 2, '0', STR_PAD_LEFT) }}</option>
                                @endforeach
                        ...
                        <p class="text-sm mb-2 mt-4">Afternoon Session</p>
                        <div class="flex items-center w-full">
                            <select class="input flex-1 rounded shadow w-full placeholder:text-gray-400 select cursor-pointer" id="afternoon_hour" name="afternoon_hour" v-model="editStart.afternoon_hour" required>
                                <option disabled selected>Pick Hour</option>
                                @foreach (range(0, 23) as $item)
                                    <option value="{{ str_pad($item, 2, '0', STR_PAD_LEFT) }}">{{ str_pad($item, 2, '0', STR_PAD_LEFT) }}</option>
                                @endforeach
                            ...
                            <span class="px-3">:</span>
                            <select class="input flex-1 rounded shadow w-full placeholder:text-gray-400 select cursor-pointer" id="afternoon_minute" name="afternoon_minute" v-model="editStart.afternoon_minute" required>
                                <option disabled selected>Pick Minute</option>
                                @foreach (range(0, 59) as $item)
                                    <option value="{{ str_pad($item, 2, '0', STR_PAD_LEFT) }}">{{ str_pad($item, 2, '0', STR_PAD_LEFT) }}</option>
                                @endforeach
                        ...
                        <div class="w-full">
                            <button class="w-full bg-green-700 text-white rounded-lg shadow-sm py-2 px-4 cursor-pointer" type="submit">Save Changes</button>
                        </div>
                    </form>
                </div>
                <form class="modal-backdrop" method="dialog">
